---
title: "Visualizing Molecular Trends in Bacterial Genomes"
author: "Jana Taha and Daniella Lato"
date: "December 2019"
output:
...
We decided to take a closer look at the selection values for \strep
...
res <- strep_omeg %>% mutate(category=cut(value.mean, breaks=c(-Inf, Inf), labels=c("#C29979")))
...
We mentioned previously, any genes that have an $\omega$ value equal to or close to 1 are most likely under neutral selection.
It is difficult to tell from the graph which genes have an $\omega$ value statistically close enough to 1 to be considered neutral.
...
, as we move away from the origin, we barely see any significant changes in the expression values. Until we are around 35 (50Kbp) away from the origin, that is when expression values starts to decrease. This is expected, as that is where we expect most of our accessory genomes to be located.
...
#Note:
When discussing the data we did not perform statistical tests to check our hypothesis (stated in **The Data** section), we instead focused on depicting these trends visually.
...
Again, we are primarily interested in $\omega$ values that are larger than zero.
We decided to add a reference line at 1 to help remind viewers about the differences between $\omega$ values that lie above and below this line.
